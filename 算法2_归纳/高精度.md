...
实现需要判断**被减数A**与**减数B**的大小，用大数-小数，并提取符号symbol.
...
if(sym == 1) greater = a, smaller = b;
else if(sym == -1) greater = b, smaller = a;
// 开始减法操作
for(int i = 1; i <= lenC; i++) {
    ...
    c[i] = greater[i] - smaller[i];
}

// 去除前导0
while(outcome[lenG] == 0) lenG--;
if(sym == -1) cout << "-";
for(int i = lenG; i >= 0; i--) cout << c[i];
```
